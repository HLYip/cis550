...
async function signup(req, res) {
  if (!req.body.username || !req.body.password || !req.body.prefer_health) {
    ...
    try {
      ...
      connection.query(
        ...
        async function (err, rows) {
          ...
          if (rows.length) {
            return res
              .status(409)
              .json({ description: "Username has already taken" });
          }
      ...
      res
        .status(500)
        .json({ description: "Internal Server Error for unforeseen reason" });
    }
  }
}
...
async function stateinfo(req, res) {
  ...
  if (req.query.state) {
    ...
    if (value == undefined) {
      connection.query(
        `with Table0 AS (SELECT business_id, name, address, R.zipcode as zipcode, stars, review_count, photo, county, city, state
    ...
    connection.query(
      `SELECT ((1-AVG(pos_pct))*0.6+AVG(vacc_pct)*0.4) AS health_score,state_abbr
        FROM Health
        GROUP BY state`,
      function (error, results, fields) {
        if (error) {
...
async function search(req, res) {
  if (!req.query.city) {
    ...
    with res as (
        ...
        from Health
    ), Res_county as (select Z2S.city,Z2S.state,R.business_id,R.name,R.address,R.zipcode,R.r_lat,R.r_long,R.stars,R.review_count,R.is_open,R.hours,R.photo,Z2S.county as county from Restaurants R join Zipcode2State Z2S on R.zipcode = Z2S.zipcode
    join Categories C on R.business_id = C.business_id
        where Z2S.city = '${city}' and category like '%${category}%')
    ...
    from Res_county Rc  join Health H on Rc.state = H.state_abbr and Rc.county = H.county
    join res join heal
...
async function getRestInfo(req, res) {
  ...
  connection.query(
    `with county_health as (
     ...
     Res_county as (select Z2S.city,Z2S.state,R.business_id,R.name,R.address,R.zipcode,R.r_lat,R.r_long,R.stars,R.review_count,R.is_open,R.hours,R.photo,Z2S.county as county from Restaurants R join Zipcode2State Z2S on R.zipcode = Z2S.zipcode )
select * from Res_county join county_health join Attributes
...
async function covid(req, res) {
  ...
  if (req.query.state) {
    connection.query(
      `SELECT report_date, SUM(case_count_change) as number
        FROM Health
